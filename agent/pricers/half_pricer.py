...
class HalfPricer(Pricer):
    def get_bidding_price(self, figgie: Figgie, suit: Suit, utils: np.ndarray) -> Optional[int]:
        ...
        return round((utils[suit.value] - minimum) / 2) + minimum

    def get_asking_price(self, figgie: Figgie, suit: Suit, utils: np.ndarray) -> Optional[int]:
        market = figgie.markets[suit.value]
        maximum = market.selling_price if market.has_seller() else round(utils[suit.value] * 2)
        return maximum - round((maximum - utils[suit.value]) / 2)
